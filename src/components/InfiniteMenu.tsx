...
class InfiniteGridMenu {
  ...
  dispose() {
    // Stop the render loop — this is the actual leak fix (the vendored `run()`
    // otherwise schedules rAF forever). We deliberately do NOT call
    // WEBGL_lose_context: under React StrictMode the effect cleanup + re-run
    // reuses the same <canvas>, and a lost context poisons the re-init. The
    // context is released when React removes the canvas on real unmount.
    this._stopped = true;
...
export default function InfiniteMenu({
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      ...
      setActiveItem(item);
      if (typeof onActiveItemChangeRef.current === 'function') {
    ...
    try {
      ...
      console.error('[InfiniteMenu] WebGL init failed:', err);
      setUnsupported(true);
      return;
    }
    ...
    return () => {
      ...
      if (sketch && typeof sketch.dispose === 'function') sketch.dispose();
    };
  }, [items, scale]);
  ...
  const handleButtonClick = () => {
    ...
    } else if (typeof onNavigateRef.current === 'function') {
